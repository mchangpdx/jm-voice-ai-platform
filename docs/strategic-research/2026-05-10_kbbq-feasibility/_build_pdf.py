...
/* Dialogue speaker colorization (Agent vs You/Caller) */
.line-agent  { color: #1e40af; }                /* deep blue — Agent */
.line-caller { color: #b45309; font-weight: 600; } /* amber/orange — You/Caller */
.spk-label   { font-weight: 700; }
"""
...
def _colorize_dialogue(html: str) -> str:
    """Inside <pre><code> blocks, color 'Agent:' and 'You:' lines for contrast.
    (대화 블록 내 화자별 색상 구분 — Agent vs 고객(You)).

    Lines starting with 'Agent:' / 'You:' set the speaker mode; subsequent
    indented continuation lines inherit that color until the next speaker.
    """
    def _wrap_block(match: "re.Match[str]") -> str:
        head, body, tail = match.group(1), match.group(2), match.group(3)
        if "Agent:" not in body and "You:" not in body:
        ...
        for raw in body.split("\n"):
            ...
            if stripped.startswith("Agent:"):
                ...
                continue
            if mode == "agent":
                out_lines.append(f'<span class="line-agent">{raw}</span>')
            elif mode == "caller":
                out_lines.append(f'<span class="line-caller">{raw}</span>')
            else:
                out_lines.append(raw)
        return head + "\n".join(out_lines) + tail

    return _PRE_RE.sub(_wrap_block, html)
...
def main() -> int:
    ...
    html_body = _colorize_dialogue(html_body)
