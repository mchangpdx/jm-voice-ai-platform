#!/usr/bin/env python3
"""Build SESSION.html + SESSION.pdf from SESSION.md.
Standalone — system python3 + python-markdown + Chrome headless.
(SESSION.md → HTML + PDF — 시스템 python3로 실행)
"""
...
BASE = sys.argv[1] if len(sys.argv) > 1 else "SESSION"
MD = HERE / f"{BASE}.md"
...
@page {
  ...
  @bottom-center {
    content: "JM Tech One · 2026-05-18 EVE · Frontend Polish Day · Page " counter(page) " / " counter(pages);
    font-size: 8pt;
    color: #6b7280;
  }
}
...
<head>
<meta charset="UTF-8">
<title>Session 2026-05-18 EVE — Frontend Polish Day — JM Tech One</title>
<style>{css}</style>
</head>
<body>
"""
