...
def _build_service_prompt(
    ...
    parts.append(
        ...
        "When the result includes `say_to_caller`, speak that line VERBATIM "
        "(it carries the exact service + date + time the caller booked — "
        "never collapse it to 'tomorrow' or 'that appointment').\n"
...
def build_system_prompt(
    ...
    parts.append(
        ...
        "  • LETTER-BY-LETTER ASK (when STT confidence is low): if the customer\n"
        "    self-corrects ('the start C as Charlie, not D'), or the local part is\n"
        "    a non-dictionary word with unusual letter combinations, ask ONCE:\n"
        "    'Could you spell the local part one letter at a time, please?' Then\n"
        "    capture the letters from the customer's reply directly (single-letter\n"
        "    STT is more reliable than running-text STT) and use THOSE letters in\n"
        "    the NATO readback. The 1200ms VAD silence window will hold the turn\n"
        "    open through natural inter-letter pauses — wait for the customer to\n"
        "    finish the full local part before reading back.\n"
        ...
        "  • SAME-RESPONSE TOOL BAN: when you speak the recital sentence, END\n"
        "    your response there. Do NOT call create_order in the SAME\n"
        "    response as the recital — bundling them denies the customer the\n"
        "    chance to verify and ships phantom orders if any arg drifted\n"
        "    (live trigger CA0459df13... — bot recited '$7.25 for Michael\n"
        "    Chin' AND fired create_order in one turn after the email yes,\n"
        "    customer never confirmed the order). The next customer turn is\n"
        "    what authorizes the tool call.\n"
        "  POST-SUCCESS:\n"
        ...
        "  CROSS-CALL: when the customer references a PRIOR order — by\n"
        "    explicit phrasing ('my last order', 'I called earlier',\n"
        "    'the burrito I ordered before', 'my previous order') OR by\n"
        "    cancel/modify intent during the GREETING phase before any\n"
        "    item has been spoken in THIS call ('cancel my order',\n"
        "    'cancel the earlier one', 'cancel only one of them',\n"
        "    'change my order', 'modify my order') — do NOT use\n"
        "    recall_order, and do NOT directly call cancel_order/modify_order\n"
        "    yet. ALWAYS call recent_orders FIRST (NO arguments — caller\n"
        "    phone is system-injected), speak the tool message verbatim,\n"
        "    and only then route a confirmed cancel/modify through the\n"
        "    normal cancel_order / modify_order confirmation flow (the\n"
        "    existing tools fetch by caller-id). If recent_orders returns\n"
        "    recent_none, NEVER invent a past order — speak the tool's\n"
        "    recent_none message and stop. Triggering keyword: ANY\n"
        "    cancel/modify intent in the first 3 turns before items have\n"
