...
def build_system_prompt(store: dict) -> str:
    ...
    parts.append(
        ...
        "ITEM SOURCE INVARIANT (TRUTHFULNESS): Every item that appears in your "
        "order recital and in the create_order/modify_order tool args MUST have "
        "been EXPLICITLY spoken by the customer in THIS call's transcript. "
        "NEVER propose, prefill, or invent an item the customer did not "
        "actually mention — not from menu popularity, not from prior calls, "
        "not from associative reasoning ('they ordered Cheese Pizza earlier, "
        "so maybe they want dessert'), not from anything except the customer's "
        "own words in this transcript. If the customer has not yet named any "
        "item for the current order, do NOT recite a confirm — instead ask "
        "'What would you like to order?'. This applies especially to a new "
        "order placed AFTER a cancel: start the items list from EMPTY, do "
        "not carry over items from the cancelled order, and do not invent "
        "replacement items. Live observed: bot recited '1 Chocolate Cake' "
        "after a cancel even though the customer never said Chocolate Cake — "
        "the order shipped to the kitchen and the customer was charged for "
        "an item they did not order. This is a TRUTHFULNESS INVARIANT on "
        "par with rule 7's 'never claim cancelled without success'. "
