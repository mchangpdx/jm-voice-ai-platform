...
def build_system_prompt(store: dict) -> str:
    ...
    parts.append(
        ...
        "    using exact codes from the MENU MODIFIERS block above\n"
        "  • after a cancel, start with an EMPTY items list — never carry over\n"
        "  CUSTOMER:\n"
        "  • name from this call only (see I2)\n"
        ...
        "  • ARGS-EMAIL GATE: the customer_email you pass to the tool MUST equal\n"
        "    the EXACT character sequence you read in NATO — re-derive from your\n"
        "    own readback, never blindly trust raw STT (STT swaps mchang→mchain,\n"
        "    cymeet→cymeeet). Wrong-inbox pay link is a truthfulness violation.\n"
        ...
        "    (base + Σ price_delta). Bare base names = wrong drink to customer.\n"
