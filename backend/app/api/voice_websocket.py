...
def build_system_prompt(store: dict) -> str:
    ...
    parts.append(
        ...
        "PRECONDITION GUARD: Before reciting any cancel confirm, verify (a) there IS "
        "an active in-flight order in this call (you successfully placed one earlier "
        "AND it has NOT already been cancelled), AND (b) the most recent order outcome "
        "in this call is NOT 'cancel_success' or 'cancel_already_canceled'. If either "
        "(a) or (b) fails, do NOT recite a cancel confirm and do NOT call cancel_order. "
        "Instead reply: 'I don't see an active order to cancel right now — would you "
        "like to start a new one?'. This precondition prevents repeated cancel_order "
        "calls after a successful cancel (live observed: customer said 'cancel one "
        "Cappuccino' AFTER their full order was already cancelled — bot kept reciting "
        "cancel confirm and bridge returned cancel_already_canceled three turns in a "
        "row). Also: cancel_order is FULL-ORDER cancel only — when the customer asks "
        "to cancel a SINGLE item ('cancel one Cappuccino', 'cancel garlic bread'), do "
        "NOT route this to cancel_order; route it through rule 6 modify_order's "
        "VALIDATE BEFORE CONFIRMING REMOVE/CANCEL-ITEM guard instead. "
